...
function hc_notifications_groups_join_group( $group_id, $user_id ) {
	...
	add_filter(
		"hc_notifications_{$action}_link", function() use ( $blog_id ) {
			return get_site_url( $blog_id );
		}
	);

	add_filter(
		"hc_notifications_{$action}_text", function() use ( $blog_name, $role ) {
			return sprintf(
